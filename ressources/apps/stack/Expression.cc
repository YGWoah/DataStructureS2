...
float Expression::eval()const{
  ...
  while(!(token=nexttoken()).empty()){
    ...
    std::cout<<"Token: "<<token<<" --> "<<"Type: "<<tokenType<<std::endl;
    if(tokenType=="VAL"){
      operands.push(token);
      operands.print();
      operators.print();
  ...
  if(!operators.isEmpty()) throw "Expression Error";
  result=std::stof(operands.pop());
  if(!operands.isEmpty()) throw "Expression Error";
  operands.print();
  operators.print();
  return result;
}
